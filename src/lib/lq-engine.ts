export type LQLevel = 'Critical' | 'Below Average' | 'Average' | 'Good' | 'Excellent'

export interface LQInput {
  attendance: number
...
export interface LQResult {
  score: number
  level: LQLevel
  breakdown: { attendance: number; homework: number; tests: number; participation: number; consistency: number }
}

const weights = { attendance: 0.2, homework: 0.2, tests: 0.3, participation: 0.15, consistency: 0.15 }

export function calculateLQ(input: LQInput): LQResult {
  const score = Math.round(
    (input.attendance * weights.attendance +
      input.homeworkCompletion * weights.homework +
      input.testScores * weights.tests +
      input.participation * weights.participation +
      input.consistency * weights.consistency) * 100
  ) / 100
  ...
  return {
    ...
    breakdown: {
      ...
      homework: input.homeworkCompletion,
      tests: input.testScores,
...
export function getLQLevel(score: number): LQLevel {
...
export function getLQColor(level: LQLevel): string {
  const colors: Record<LQLevel, string> = {
...
// -- LQ Paper Generator (Long Question / Subjective Engine) --
